...
By setting :math:`F(\mathbf{x}) = - \mathbf{M}\mathbf{x} - \mathbf{q}` and :math:`\mathcal{S} = \mathbb{R}_{\geq 0}` it can be readily verified that each solution for :math:`(\mathbf{x} - \mathbf{x}^*)^\top F(\mathbf{x}^*) \geq 0` is also a solution for :eq:`complementarity`. 
