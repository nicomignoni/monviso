...
As shown in Proposition 2 in [1]_, the receding horizon open-loop Nash equilibria (NE) can be reformulated as a non-symmetric variational inequality. Specifically, consider a set of agents :math:`\mathcal{N} = \{1,\dots,N\}` characterizing a state vector :math:`\mathbf{x}[t] \in \mathbb{R}^n`, whose (linear) dynamics is described as
...
Following Definition 1 in [1]_, the sequence of input :math:`\mathbf{u}^*_i \in \mathcal{U}_i(\mathbf{x}_0,\mathbf{u}_{-i})`, for all :math:`i \in \mathcal{N}`, characterizes an open-loop NE iff
...
.. literalinclude:: ../../examples/linear-quadratic-game.py
   :language: python
